...
public class PedidoServiceImpl implements PedidoService {
    ...
    @Override
    public void procesarPago(Long pedidoId, DetallesPago detallesPago) {
        Pedido pedido = pedidoRepository.buscarPorId(pedidoId);

        if (pedido.getEstado() != EstadoPedido.PENDIENTE) {
            throw new IllegalStateException("El pedido no está en un estado válido para ser pagado.");
        }

        boolean pagoExitoso = paymentGateway.procesarPago(detallesPago);

        if (pagoExitoso) {
            this.pagarPedido(pedidoId);
            notificationService.notificarPagoExitoso(pedido);
        } else {
